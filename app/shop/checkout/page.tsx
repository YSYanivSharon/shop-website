...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    const updateShipping = async () =>
      setShippingPrice(await getShippingPrice(address as Address));
    updateShipping();
  }, [address]);
  ...
  return (
    <form onSubmit={onPay} className="max-w-2xl mx-auto p-6 space-y-6">
      ...
      <div>
        ...
        {cart.length > 0 &&
          cart.map((entry, index) => renderEntry(entry, index))}
      </div>
      ...
      <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-xl shadow space-y-3">
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          {errors.expirationMonth && (
            <p className="text-red-500 text-sm">{errors.expirationMonth}</p>
          )}
          ...
          {errors.expirationYear && (
            <p className="text-red-500 text-sm">{errors.expirationYear}</p>
          )}
